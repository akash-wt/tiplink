...
impl ResponseError for Error {
    fn error_response(&self) -> HttpResponse {
        match self {
            Error::WrongNetwork(msg) => HttpResponse::BadRequest().body(format!("Wrong network: {}", msg)),
            Error::BadBase58(_) | Error::WrongKeyPair(_) | Error::DeserializationFailed { .. } => {
                HttpResponse::BadRequest().body(self.to_string())
            }
            Error::AirdropFailed(_)
            | Error::RecentHashFailed(_)
            | Error::ConfirmingTransactionFailed(_)
            | Error::BalaceFailed(_)
            | Error::SendTransactionFailed(_) => {
                HttpResponse::InternalServerError().body(self.to_string())
            }
            Error::MismatchMessages
            | Error::InvalidSignature
            | Error::KeyPairIsNotInKeys => HttpResponse::BadRequest().body(self.to_string()),
        }
    }
}